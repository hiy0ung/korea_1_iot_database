...
-- 2. 'SEOUL'에 거주하는 회원의 이름과 회원 등급을 조회하시오.
select `name`, `grade`, `area_code` -- 확인을 위해 area_code 출력
from `members`
where `area_code` = 'SEOUL';

-- 3. 회원등급이 'Gold' 이상인 회원의 이름과 가입일 조회하시오.
select `name`, `join_date`, `grade` -- 확인을 위해 grade 출력
from `members`
where `grade` in('Gold', 'Platinum', 'Diamond');

select `name`, `join_date`, `grade` -- 확인을 위해 grade 출력
from `members`
where `grade` not in('Gold', 'Platinum', 'Diamond'); -- 포함하지 않는 것(Bronze, Silver 출력)
...
-- 8. 회원별로 구매한 총 금액(amonut의 합)을 조회하시오.
