...
/*
    ...
    -- 열 이름을 생략할 경우 values 키워드 뒤의 값들의 순서는 
...
create table `example02` (
	-- 컬럼명 데이터타입 primary key auto_increment (pk랑 auto~랑 순서 상관 X)
...
update `example02`
set name='선하영'; -- 전체 name 데이터값이 '선하영'으로 바뀜
select * from `example02`;

select * from `example03`;

-- safe update mode를 해제하는 코드 (전체적으로 모든 데이터 못바꾸게 함 / 중요한 데이터를 한 번에 바꿀 수 없게 함?)
...
-- 위 코드에서는 id값을 2로 나눈 값이 1인지 여부를 확인

select * from `example03`;

# cf) 여러 컬럼의 값을 한 번에 설정하는 경우 콤마(,)로 분리하여 나열
